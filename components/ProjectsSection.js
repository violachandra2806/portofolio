  ...
  return (
    <Box component="section" id="projects" sx={{ minHeight: "100vh", padding: { xs: "56px 0", sm: "88px 0" }, background: "linear-gradient(180deg, var(--bg) 0%, color-mix(in srgb, var(--bg) 58%, #f6e8dc) 50%, #f6e8dc 100%)", scrollMarginTop: 96 }}>
      ...
      <Box sx={{ position: "relative", width: { xs: "calc(100% - 32px)", sm: "100%" }, margin: "0 auto", "&:hover .project-rail-arrow": { opacity: 1, visibility: "visible", pointerEvents: "auto" } }}>
        <IconButton className="project-rail-arrow" sx={{ display: { xs: "flex", md: "none" }, position: "absolute", top: "42%", left: -8, zIndex: 2, width: 40, height: 40, transform: "translateY(-50%)", opacity: 0, visibility: "hidden", pointerEvents: "none", border: "1px solid var(--line)", background: "var(--panel)", color: "var(--text)", transition: "opacity .25s ease, visibility .25s ease, transform .25s ease", "&:hover": { transform: "translateY(-50%) scale(1.08)" } }} onClick={() => scrollProjectRail(-1)} aria-label="Previous project"><ChevronLeft /></IconButton>
        <IconButton className="project-rail-arrow" sx={{ display: { xs: "flex", md: "none" }, position: "absolute", top: "42%", right: -8, zIndex: 2, width: 40, height: 40, transform: "translateY(-50%)", opacity: 0, visibility: "hidden", pointerEvents: "none", border: "1px solid var(--line)", background: "var(--panel)", color: "var(--text)", transition: "opacity .25s ease, visibility .25s ease, transform .25s ease", "&:hover": { transform: "translateY(-50%) scale(1.08)" } }} onClick={() => scrollProjectRail(1)} aria-label="Next project"><ChevronRight /></IconButton>
        <Container ref={projectRailRef} component={Box} onScroll={updateActiveProject} sx={{ display: { xs: "flex", md: "grid" }, gridTemplateColumns: "1fr", alignItems: "stretch", gap: { xs: 2, md: 3 }, width: { xs: "calc(100% - 32px)", md: "100%" }, margin: { xs: 0, md: "0 auto" }, overflowX: { xs: "auto", md: "visible" }, flexWrap: { xs: "nowrap", md: "initial" }, scrollSnapType: { xs: "x mandatory", md: "none" }, scrollBehavior: { xs: "smooth", md: "auto" }, scrollbarWidth: "thin", padding: { xs: "0 16px 8px", md: 0 }, "@media (min-width: 900px)": { gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))" } }}>
        ...
        <Stack direction="row" sx={{ display: { xs: "flex", md: "none" }, justifyContent: "center", gap: .75, marginTop: 1.5 }} aria-label="Project carousel pagination">
