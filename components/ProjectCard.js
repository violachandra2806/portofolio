...
export default function ProjectCard({ title, description, tags, badge, badgeColor, image, onOpen }) {
  ...
  return (
    <Card elevation={0} sx={{ position: "relative", flex: { xs: "0 0 min(82vw, 340px)", sm: "initial" }, scrollSnapAlign: "center", overflow: "hidden", width: { xs: "min(82vw, 340px)", sm: "100%" }, height: { xs: 430, sm: 560 }, border: "1px solid var(--line)", borderRadius: { xs: "22px", sm: "28px" }, background: "var(--panel)", cursor: "pointer", transition: "transform .28s ease, box-shadow .28s ease", "&:hover": { transform: "translateY(-6px)", boxShadow: "0 20px 40px var(--shadow)" } }}>
      ...
      <Box sx={{ position: "relative", flex: "0 0 auto", width: "100%", aspectRatio: { xs: "16 / 9", sm: "16 / 10" }, overflow: "hidden", borderBottom: "1px solid var(--line)" }}>
      ...
      <CardContent sx={{ display: "flex", flex: 1, minHeight: 0, flexDirection: "column", padding: "20px 18px 18px" }}>
        ...
        <Typography component="p" sx={{ margin: 0, color: "var(--muted)", fontSize: "var(--font-size-detail)", lineHeight: 1.6, display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>{description}</Typography>
            <Stack direction="row" sx={{ display: "flex", maxHeight: { xs: 64, sm: "none" }, overflow: "hidden", flexWrap: "wrap", alignContent: "flex-start", gap: 1, marginTop: 1.75 }}>
          {tags.map((tag) => (
