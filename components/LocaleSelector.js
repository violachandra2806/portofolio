...
export default function LocaleSelector({ value, onChange, label }) {
  return (
    <Stack direction="row" sx={{ display: "inline-flex", alignItems: "center", gap: 1.5, color: "var(--muted)", fontSize: "var(--font-size-xs)", lineHeight: 1, "@media (max-width: 640px)": { position: "fixed", left: 0, top: "50vh", zIndex: 60, transform: "translateY(-50%)", flexDirection: "column", gap: 0.75, padding: "10px 8px 10px 10px", border: "1px solid var(--line)", borderLeft: 0, borderRadius: "0 10px 10px 0", background: "rgba(255, 252, 246, .94)", boxShadow: "0 8px 24px rgba(76, 67, 54, .14)", backdropFilter: "blur(12px)" } }}>
      <Typography component="span" sx={{ whiteSpace: "nowrap", lineHeight: 1.2, "@media (max-width: 640px)": { display: "none" } }}>{label}</Typography>
      <ButtonBase
        aria-label="Switch language"
        onClick={() => onChange(value === "en" ? "id" : "en")}
        sx={{ display: "none", color: "var(--primary)", borderRadius: "50%", "@media (max-width: 640px)": { display: "inline-flex" } }}
      >
      ...
      <ButtonBase
        ...
        onClick={() => onChange(value === "en" ? "id" : "en")}
        sx={{ display: "none", color: "var(--text)", fontSize: "11px", fontWeight: "var(--weight-bold)", writingMode: "vertical-rl", transform: "rotate(180deg)", "@media (max-width: 640px)": { display: "inline-flex" } }}
      >
